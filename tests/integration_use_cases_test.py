...
class UseCaseTest(unittest.TestCase):
    ...
    def test_02_first_user_joined(self):
        ...
        with self.engine.database_session():
            ...
            (
                users_events,
                users_meetings_events,
            ) = self.engine.users_meetings_events_by_user_id(internal_user_id)
            (
                meetings,
                meetings_meetings_events,
            ) = self.engine.meetings_meetings_events_by_meeting_id(internal_meeting_id)
            ...
            self.assertEqual(
                attendees[0]["internal_user_id"],
                event[0]["data"]["attributes"]["user"]["internal-user-id"],
            )
            self.assertEqual(
                attendees[0]["external_user_id"],
                event[0]["data"]["attributes"]["user"]["external-user-id"],
            )
            self.assertEqual(
    ...
    def test_03_second_user_joined(self):
        ...
        with self.engine.database_session():
            ...
            self.assertEqual(
                attendees[1]["internal_user_id"],
                event[0]["data"]["attributes"]["user"]["internal-user-id"],
            )
            self.assertEqual(
                attendees[1]["external_user_id"],
                event[0]["data"]["attributes"]["user"]["external-user-id"],
            )
            self.assertEqual(
    ...
    def test_09_second_user_presenter_unassigned(self):
        ...
        with self.engine.database_session():
            ...
            self.assertEqual(
                attendees[0]["is_presenter"], False
            )  # Second user is now the first in the list.
